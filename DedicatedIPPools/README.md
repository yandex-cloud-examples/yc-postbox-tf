...
Пример создаёт пул выделенных IP-адресов, перемещает в него выделенные IP-адреса вашего облака и создаёт конфигурацию отправки, привязанную к этому пулу. Письма, отправленные с указанием этой конфигурации, уходят только с адресов пула.
...
1. **Выделенные IP-адреса уже выданы вашему облаку.** Terraform их не создаёт: он только раскладывает по пулам уже выданные адреса. Посмотреть список можно так:
...
2. **Каталог, который вы укажете в `folder_id`, находится в том же облаке, что и эти адреса.** Пример создаёт в этом каталоге сервисный аккаунт и работает с Postbox от его имени, поэтому пул, привязки адресов и конфигурация отправки появятся в облаке именно этого каталога. Вывод `yc config get folder-id` доказательством не является: каталог по умолчанию может относиться к другому облаку и при этом содержать сервисный аккаунт с ролью `postbox.admin` — например, `postbox-admin`, созданный примером EasyDKIM или BYODKIM. Убедитесь, что в `folder_id` стоит каталог того облака, чьи адреса вы только что увидели в `aws sesv2 get-dedicated-ips`; посмотреть облако каталога можно командой `yc resource-manager folder get <folder_id>`.
...
Выделенные IP-адреса при этом никуда не деваются: они остаются в вашем облаке и возвращаются в пул `ses-default-dedicated-pool`. Проверить можно так:
...
**`apply` падает с `Could not find dedicated IP … under this account` — значит, неверный каталог, а не адрес.** Ошибка называет IP, но причина в другом: сервисный аккаунт создан в каталоге чужого облака, и выделенного адреса в этом облаке действительно нет.

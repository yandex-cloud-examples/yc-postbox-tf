...
2. **Каталог, который вы укажете в `folder_id`, принадлежит тому же тенанту Postbox, что и эти адреса.** Пример создаёт в этом каталоге сервисный аккаунт и работает с Postbox от его имени, поэтому пул, привязки адресов и конфигурация отправки появятся в тенанте именно этого каталога. Вывод `yc config get folder-id` доказательством не является: каталог по умолчанию может лежать в том же облаке и даже содержать сервисный аккаунт с ролью `postbox.admin` — например, `postbox-admin`, созданный примером EasyDKIM или BYODKIM, — но относиться к другому тенанту Postbox. Убедитесь, что в `folder_id` стоит каталог того тенанта, чьи адреса вы только что увидели в `aws sesv2 get-dedicated-ips`; если сомневаетесь — уточните каталог у владельца тенанта.
